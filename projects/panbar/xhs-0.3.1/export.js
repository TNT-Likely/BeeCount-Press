...
/// 模板配置 — 新增卡片时在这里加一项即可。
const TEMPLATES = [
  { id: '01-cover',           name: '01 · 总封面 PanBar 0.3.x',            file: '01-cover.html' },
  { id: '02-three-markets',   name: '02 · 真实截图 · 三市一站',             file: '02-three-markets.html' },
];
...
// 持久化选项
// ============================================================
const STORAGE_KEY = 'panbar-xhs-selections';
...
async function captureTemplateBlob(template) {
  ...
  try {
    ...
    if (!idoc) {
      throw new Error(
        `iframe contentDocument 为 null — 多半是用 file:// 直接打开 ` +
        `(浏览器把同目录的 file 当跨源)。请用 http server,例如:\n` +
        `cd ${location.pathname.replace(/\/[^\/]+$/, '')} && python3 -m http.server 8765\n` +
